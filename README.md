...
- 直播间内用户头像
- 排行榜
- 冠名礼物（礼物标识 由xxx冠名的礼物）（有人发issue或者哪天想起来再做）
...
- 底部礼物combo  
目前只遇到过一次，此combo不会携带用户信息，只有热度极高的直播间才会出现。携带信息的礼物信息在弹幕区的悬浮区内，数据去重很困难（数据会和聊天区和底部重合）。因此暂不考虑
...
- 礼盒的爆出的数量是不是固定
- 是否可能出现不显示数量的情况  
在当前收集到的样例中只有类似于这样的文本段：
```json
["<userName>", "投喂", "<boxName>", "爆出", "<giftName>", "x1"]
```
并且显示数值的位置可能有多个，而目前收集到的样本只有一种。因此目前我是假定:
1. 两个数量显示位置只使用其中一个类名包含固定内容的元素
2. 此元素一定包含数量内容，而非用空来指代一个
...
- 悬浮区逻辑：悬浮区礼物数量显示 目前抓到的案例都很牛逼，是使用带背景图片的节点实现（例如100就是1 0 0三张图片拼接），以及连击数更新机制未知，不确定是否易于实现
